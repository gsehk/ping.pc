    ...
    public function rewards(Request $request)
    {
        ...
        if($request->getinfo){
            ...
            switch ($type) {
                ...
                case 'news':
                        $data['app'] = '资讯';
                        $data['rewards'] = createRequest('GET', '/api/v2/news/'.$post_id.'/rewards', $params);
                    break;

                case 'answer':
                        $data['app'] = '问答';
                        $data['rewards'] = createRequest('GET', '/api/v2/question-answers/'.$post_id.'/rewarders', $params);
                    break;

                default:
                    $data['app'] = '动态';
                    $data['rewards'] = createRequest('GET', '/api/v2/feeds/'.$post_id.'/rewards', $params);
                    break;
            }
