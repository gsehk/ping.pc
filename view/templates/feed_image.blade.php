...
@if (isset($image['paid']) && !$image['paid'])
    <div class="locked_image" @if (isset($count) && $count == 'one') style="{{ $style }};position:relative" @endif data-node="{{ $image['paid_node'] }}" data-amount="{{ $image['amount'] / 100 }}" data-file="{{ $image['file'] }}" data-original="{{ getImageUrl($image, $width, $height) }}">
